/**
 * Page-context reader (PRD-007).
 *
 * Resolves the shopper's current page snapshot from one of:
 *   1. `config.pageContext` passed to `init()` (preferred — host page builds it)
 *   2. `data-*` attributes on the widget mount node (Shopify Liquid block)
 *   3. `data-*` attributes on a host-supplied selector
 *
